...
def _main( ):
    ...
    for i in range(number_of_simulation):
        ...
        #print A, X, Y
        '''Ylog,Xlog,A = s.cholesky_nlblock( number_features, number_samples , number_blocks, fVal = .6 , Beta = 3.0, link = "parabola" )
        ...
        X = np.concatenate((X, Xlog), axis=0)
        Y = np.concatenate((Y, Ylog), axis=0)
        _ ,_,A = s.double_cholesky_block( 8, 150 , 2, fVal = 2.6 , Beta = 3.0 )#, link = "line" )'''
