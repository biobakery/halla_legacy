...
def _main( ):
    ...
    #c1 = halla.discretize( [0.0215621,-0.02010377,0.02830424,0.04457466,0.007305178,-0.01189224,0.02660646,0.03015426,0.03438763,0.0143625,0.004416951,0.009470937,0.03856011,-0.01198532,-0.01326471,0.01962859,0.02473509,-0.02637184,0.005602601,0.002466046,0.02157657,0.004477183,0.04505282,-0.01116779,-0.02437186,0.01089451,0.0317678,0.04590994,0.01923795,0.04317521,0.03573549,0.03655337,0.05519025,-0.230087,0.03434896,-0.01633808,0.002737079,0.007142806,0.0386229,0.01314257,0.03141105,0.0187987,0.006444907,0.01119644,0.03749342,0.00277615,0.02655672,0.01870941,-0.02943997,0.01387386,-0.01411101,0.01987697,0.03579529,0.021342,-0.04742493,0.0392137,-0.01370008,0.02048916,0.02549812,0.01614528,-0.02424764,-0.01992867,0.01237199,-0.5424096,0.05314519,-0.007888051,0.0368969,0.01788898,0.003059311,0.0045881,0.01406968,0.02653482,0.01280068,-0.006655993,-0.03057622,0.03033359,0.009197163,0.002621455,0.02560563,0.04956631,-0.000687241,0.01272528,0.02740269,0.008084011,0.01931709,0.03301485,0.04636154,0.006759301,0.002624012,0.02433382,-0.01293969,-0.003585531,-0.0156252,-0.7295377,0.009627441,0.0318877,-0.02107784,0.02149054,0.03912302,0.02477664])
    #c2 = halla.discretize( [-3.0376585,2.37714672,-3.25607609,-2.11179039,-2.465046945,0.47053029,0.13015415,-4.02672560,-3.22029433,8.3996607,8.650537896,-1.108326476,-3.52504916,0.70437578,0.56357802,-0.66511993,1.31827280,2.51404768,0.733782620,1.244582645,-0.10594100,-0.448905774,-0.11128072,0.41884713,2.54330121,-0.60716234,-2.7639837,-2.33204482,-2.51246756,-3.71707818,-3.15297971,-2.70554155,0.28557436,7.336971,-0.17401843,1.24366330,-2.961089981,-0.079492427,-2.7655489,-0.57376303,3.78103585,-2.1618021,1.763420229,-2.43038445,6.16317737,0.30375585,-3.12440374,-1.70559427,1.02567397,-0.76110143,1.77614347,0.22342055,-1.19317704,3.176290,0.04829607,-1.4825659,-1.25732206,-1.01950788,0.92111446,3.63381621,0.35921269,0.35226475,-4.90703900,6.4570819,-5.45468379,-0.313783927,6.5795907,-2.95806511,0.159081159,-2.1760431,0.66200608,0.67783888,-1.82243712,0.879941716,2.61734069,6.80540421,9.045639613,0.125071359,-3.46621621,-2.95947763,-1.185410561,0.37339500,-1.54611023,-0.969250459,0.33815023,-1.59414254,-1.76552169,-1.037236284,-3.083432872,-2.87041502,1.74680487,-2.955956719,-0.6485849,7.4752446,-4.355770427,8.1255674,0.87406149,2.03783677,-5.04207971,-3.96772628])
    #print"NMI", distance.NormalizedMutualInformation( c1, c2 ).get_distance()
    #return 
    q_cutoff = {.1}
...
#       X = array([[1,1,1,3],[1,1,1,3],[1,1,1,3],[1,1,1,3]])
        ...
        X = np.concatenate((X, Xlog), axis=0)
        Y = np.concatenate((Y, Ylog), axis=0)
        Xlog,Ylog,A = s.cholesky_nlblock( number_features, number_samples , number_blocks, fVal = .6 , Beta = 3.0)#, link = "sine" )
        X = np.concatenate((X, Xlog), axis=0)
        X = np.concatenate((X, Ylog), axis=0)'''
        
        #print A, X, Y'''
        '''Ylog,Xlog,A = s.cholesky_nlblock( number_features, number_samples , number_blocks, fVal = 2.6 , Beta = 3.0, link = "sine" )
        X = np.concatenate((X, Xlog), axis=0)
        Y = np.concatenate((Y, Ylog), axis=0)'''
        ''' 
        Ylog,Xlog,A = s.cholesky_nlblock( number_features, number_samples , number_blocks, fVal = 2.0 , Beta = .30)#, link = "log" )
        X = np.concatenate((X, Xlog), axis=0)
        Y = np.concatenate((Y, Ylog), axis=0)
        Xlog,Ylog,A = s.cholesky_nlblock( number_features, number_samples , number_blocks, fVal = 2.0 , Beta = 3.0, link = "sine" )
        X = np.concatenate((X, Xlog), axis=0)
        Y = np.concatenate((Y, Ylog), axis=0)
        Ylog,Xlog,A = s.cholesky_nlblock( number_features, number_samples , number_blocks, fVal = 2.0 , Beta = 3.0, link = "parabola" )
        X = np.concatenate((X, Xlog), axis=0)
        Y = np.concatenate((Y, Ylog), axis=0)
        _ ,_,A = s.double_cholesky_block( 16, 150 , 4, fVal = 2.6 , Beta = 3.0 )#, link = "line" )
        A = np.zeros((20,20))
        A[0:4,0:4]= 1.0
        A[16:20,16:20]= 1.0
        print A'''
        ...
        #X,Y,A = s.cholesky_nlblock( number_features, number_samples , number_blocks, fVal = .6, Beta = 3.0, link = "log" )
