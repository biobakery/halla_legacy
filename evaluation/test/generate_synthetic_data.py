...
#*   This program generates synthetic data and stores its results in a JSON file that will be  read    *
#*      by the test_association program                                                                *
#*******************************************************************************************************
import matplotlib 
matplotlib.use("Agg")
...
def _main( strFile, strFileAlpha, iRow, iCol, strMethod, iIter, fSparsity, fNoise, strSpike, strBase, bParam, iPval, fPermutationNoise, bException, strMultiProcessing ):
	...
	def __main( ):
		...
		#******************************************************************************************************
		dEmp = dict()
		dEmp =  Convert_array_to_list_and_push_to_dict(A_flatten, dEmp, 'A_flatten' )
		dEmp =  Convert_array_to_list_and_push_to_dict(A_emp_flatten, dEmp, 'A_emp_flatten')
		dEmp['iIter'] = iIter
		dEmp['strFile'] = strFile
		dEmp['strTitle'] = strTitle
		dEmp['strFileAlpha'] = strFileAlpha
		dEmp['strTitleAlpha'] = strTitleAlpha
 
 		strOutfileName = args.strosj + ".json"
		if  args.strosj == "data":
			strOutfileName =  strFile[:-3] + "json"

		with open(strOutfileName , 'w') as outfile:
			json.dump(dEmp, outfile)
